...
// - 販売データが存在すれば (JAN,YYYY-MM) / JAN で粗利JOINし、手残り（粗利−廃棄原価）。
// - 表描画 / TOP20棒グラフ / CSV(Shift_JIS優先) エクスポート。
// 依存: PapaParse, Plotly（index.html でCDN読込）
...
(function () {
  ...
  const WASTE = {
    raw: [],
    fields: [],
    aggrMonth: {}, // { `${jan}|${ym}`: { jan, ym, qty, wasteCost, name, dept, gp, net } }
    aggrTotal: {}, // { jan: { jan, qty, wasteCost, name, dept, gp, net } }
    meta: { months: [] },
  };

  // ---------- utilities ----------
  const el = function(id){ return document.getElementById(id); };
  // ← スプレッド構文を使わずにログを出す（互換性のため）
  function log(){
  ...
  const normKey = function(k){ return String(k || "").replace(/^\uFEFF/, "").trim(); }; // BOM除去 + trim

  function pickCol(cols, candidates) {
    const set = new Set(cols);
    for (var i=0;i<candidates.length;i++){ if (set.has(candidates[i])) return candidates[i]; }
  ...
  async function loadWasteCSV(file) {
    let txt = "";
    try { txt = await readAsTextWithEncoding(file, "shift_jis"); }
    catch { txt = await readAsTextWithEncoding(file, "utf-8"); }
    ...
    const data = (res.data || []).map(function(row){
      const out = {};
      for (const k in row) out[normKey(k)] = row[k];
      return out;
    });
    return { data, fields };
  }
  ...
  function renderTable(mode) {
    ...
    if (mode === "month") {
      const rows = Object.values(WASTE.aggrMonth).sort(function(a,b){
        return (a.ym===b.ym)? a.jan.localeCompare(b.jan) : a.ym.localeCompare(b.ym);
      });
      ...
      const rows = Object.values(WASTE.aggrTotal).sort(function(a,b){ return a.jan.localeCompare(b.jan); });
      html += '<table class="table"><thead><tr>'
  ...
  function renderPlot(mode) {
    ...
    if (mode === "month") {
      ...
      const arr = Object.values(WASTE.aggrMonth)
        .filter(function(r){ return r.ym === last; })
        .sort(function(a,b){ return b.wasteCost - a.wasteCost; })
        .slice(0, 20);
      labels = arr.map(function(r){ return r.name ? (r.name + "\n(" + r.jan + ")") : r.jan; });
      values = arr.map(function(r){ return Math.round(r.wasteCost); });
      title = "廃棄原価TOP20（" + last + "）";
    } else {
      const arr = Object.values(WASTE.aggrTotal)
        .sort(function(a,b){ return b.wasteCost - a.wasteCost; })
        .slice(0, 20);
      labels = arr.map(function(r){ return r.name ? (r.name + "\n(" + r.jan + ")") : r.jan; });
      values = arr.map(function(r){ return Math.round(r.wasteCost); });
    ...
    window.Plotly.newPlot(div, [{ type: "bar", x: labels, y: values }], { title: title, margin: { t: 40, l: 40, r: 10, b: 100 } });
  }
  ...
  function exportCSV() {
    ...
    if (mode === "month") {
      ...
      rows = Object.values(WASTE.aggrMonth)
        .sort(function(a,b){ return (a.ym===b.ym)? a.jan.localeCompare(b.jan) : a.ym.localeCompare(b.ym); })
        .map(function(r){
      ...
      rows = Object.values(WASTE.aggrTotal)
        .sort(function(a,b){ return a.jan.localeCompare(b.jan); })
        .map(function(r){
    ...
    const fname = (mode === "month" && last) ? ("waste_report_" + last + ".csv") : "waste_report_all.csv";
  ...
  function wire() {
    ...
    const expBtn  = el("wasteExportBtn");

    if (!pickBtn || !input) { log("UI not found", { pickBtn: !!pickBtn, input: !!input }); return; }
    ...
    input.addEventListener("change", async function(e){
      ...
      try {
        const res = await loadWasteCSV(file);
        WASTE.raw = res.data || [];
    ...
    document.addEventListener("click", function(ev){
      const t = ev.target;
      if (!t) return;
      if (t.id === "wastePickBtn" || (t.closest && t.closest("#wastePickBtn"))) {
        const input2 = el("wasteFile");
